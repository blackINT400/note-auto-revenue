...
※ グローバルなキー監視は使わない設計なので、アクセシビリティ許可は不要です。
"""
...
try:
    ...
    from Cocoa import (
        ...
        NSSlider, NSColor, NSBezierPath, NSScreen, NSEvent, NSTimer, NSMakeRect,
...
class AppController(NSObject):
    ...
    def init(self):
        ...
        if self is None:
            return None
        self.draw_mode = False   # 起動時は「操作モード」= マウス素通り(閉じ込め防止)
        self.tool_buttons = {}
        return self
    ...
    def build(self):
        ...
        self.window.setIgnoresMouseEvents_(True)   # 最初は素通り
        ...
        self._install_killswitch()
        self._highlight_tool("pen")
        self._apply_mode()

        print("▶ 起動しました（今は『操作モード』= マウスは自由に使えます）")
        print("   ・描く   : 上部ツールバーの「描画」ボタン、または F8 キー")
        print("   ・戻る   : 描画中に F8 キーでいつでも操作モードへ")
        print("   ・終了   : Esc / Cmd+Q / このターミナルで Ctrl+C")
    ...
    def _apply_mode(self):
        ...
        if self.draw_mode:
            self.mode_btn.setTitle_("● 描画中(F8で操作)")
            self.window.makeKeyAndOrderFront_(None)
            NSApplication.sharedApplication().activateIgnoringOtherApps_(True)
        else:
            self.mode_btn.setTitle_("○ 操作中(F8で描画)")
        self.panel.orderFrontRegardless()

    # Ctrl+C(SIGINT)で確実に終了できるようにする安全装置
    @objc.python_method
    def _install_killswitch(self):
        signal.signal(signal.SIGINT, lambda *a: NSApplication.sharedApplication().terminate_(None))
        # ランループにPythonを定期的に走らせ、保留中のSIGINTを処理させる
        self._timer = NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_(
            0.3, self, "_tick:", None, True)
    ...
    def _install_hotkeys(self):
        def handler(event):
            ...
            if kc == 53:                          # Esc → 終了
                NSApplication.sharedApplication().terminate_(None)
                return None
            if cmd and chars == "q":              # Cmd+Q → 終了
                NSApplication.sharedApplication().terminate_(None)
                return None
            if kc == 100:                         # F8 → モード切替
                self.toggleMode_(self.mode_btn)
                return None
            if cmd and chars == "z":              # Cmd+Z → 元に戻す
                self.view.undo()
                return None
