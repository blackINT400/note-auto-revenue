...
処理:
  1. Pixabay Video APIでmood別動画素材を取得（30〞120秒）
...
# mood別Pixabay検索キーワード（優先順）
MOOD_KEYWORDS: dict[str, list[str]] = {
    "lofi":      ["fireplace cozy", "rain window", "candle flame", "rain night"],
    "jazz":      ["fireplace cozy", "rain window", "candle flame", "jazz cafe"],
    "ambient":   ["ocean waves", "forest rain", "lake morning", "nature calm"],
    "cyberpunk": ["city rain night", "neon lights", "rain city", "night city"],
    "piano":     ["candle fireplace", "rain window", "fireplace cozy", "candle"],
    "focus":     ["forest morning", "rain desk", "rain window", "nature forest"],
    "r&b":       ["fireplace cozy", "rain window", "candle flame", "night lounge"],
    "smooth":    ["fireplace cozy", "rain window", "ocean sunset", "candle"],
    "sleep":     ["ocean waves", "rain window", "forest rain", "fireplace cozy"],
    "relax":     ["ocean waves", "forest rain", "lake morning", "fireplace cozy"],
}
DEFAULT_KEYWORDS = ["fireplace cozy", "rain window", "ocean waves", "forest nature"]
...
def _search_pixabay_video(keyword: str, api_key: str) -> str | None:
    params = {"key": api_key, "q": keyword, "video_type": "film",
              "min_width": 1280, "per_page": 20, "safesearch": "true"}
    try:
        r = requests.get(PIXABAY_API_URL, params=params, timeout=30)
        r.raise_for_status()
        hits = r.json().get("hits", [])
        filtered = [h for h in hits if 30 <= h.get("duration", 0) <= 120]
        if not filtered:
            filtered = [h for h in hits if h.get("duration", 0) > 0]
        if not filtered:
            return None
...
def _get_pixabay_video(assets_dir: Path, genre: str) -> Path | None:
    api_key = os.environ.get("PIXABAY_API_KEY")
    if not api_key:
        logger.warning("PIXABAY_API_KEY未設定 -> スキップ")
        return None
...
def generate_bgm_video(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Step 2: Pixabayループ動画
        logger.info(f"Pixabay動画取得中... (genre={genre})")
        bg_video = _get_pixabay_video(assets_dir, genre)
